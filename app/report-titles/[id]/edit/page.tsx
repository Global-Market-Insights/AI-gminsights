...
import { ReportTitle, ReportTitleFormData } from '@/types';

export default function EditReportTitlePage({ params }: { params: { id: string } }) {
  ...
  const fetchReportTitle = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/report-titles/${params.id}`);
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          deadline: title.deadline.split('T')[0], // Convert to YYYY-MM-DD format
          tags: title.tags || [],
          estimatedPages: title.estimatedPages,
          clientName: title.clientName || '',
          notes: title.notes || ''
        });
      } else {
        alert('Report title not found');
        router.push('/report-titles');
      }
    } catch (error) {
      console.error('Error fetching report title:', error);
      alert('Failed to load report title');
      router.push('/report-titles');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert('Failed to update report title: ' + data.error);
      }
    } catch (error) {
      ...
      <div className="bg-gray-100 min-h-screen">
        ...
        <main className="p-6">
          <div className="max-w-4xl mx-auto text-center py-8">
            ...
            <p className="text-gray-500 mt-2">Loading report title...</p>
          </div>
        </main>
      </div>
    );
      ...
      <main className="p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Edit Report Title</h1>
              <p className="text-gray-600 mt-1">Update the report title information</p>
            </div>
            <Link
              href="/report-titles"
